    ...
    <PublicShell>
      <section className="mb-[14px] grid grid-flow-dense grid-cols-1 gap-[14px] sm:grid-cols-2 sm:auto-rows-[calc(50cqw_-_7px)] lg:grid-cols-4 lg:auto-rows-[calc(25cqw_-_10.5px)]">
        ...
        <article className="relative col-span-1 row-span-1 min-w-0 overflow-auto rounded-[12px] bg-[var(--card)] p-6 sm:row-span-2">
          <h2 className="lowercase mb-6 text-[18px] font-bold leading-[1.16] tracking-[-0.03em]">
          ...
          <ol className="m-0 grid list-none gap-[18px] p-0">
            {[
              ...
              <li className="gap-3" key={number}>
                {/* <span className="text-xs font-bold text-[var(--accent)]">{number}</span> */}
                <div>
                  <strong className="text-[14px] uppercase">{title}</strong>
                  <p className="mb-0 mt-1 text-[14px] leading-[1.5] text-[var(--text-soft)]">
                    {text}
                  </p>
                </div>
              </li>
                ...
                <article className="relative col-span-2 row-span-1 min-w-0 overflow-hidden rounded-[12px] bg-[var(--card)] p-6">
          <h2 className="mb-3 text-[18px] font-bold lowercase tracking-[0.04em]">
            core tools.
          </h2>
          <div className="flex flex-wrap gap-4">
            {homepage.about.skills.map((skill) => (
              <span
                className="inline-flex text-[14px] leading-none text-[var(--text-soft)]"
                key={skill}
              >
                • {skill}
              </span>
