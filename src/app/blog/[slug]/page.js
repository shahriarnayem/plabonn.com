...
export default async function BlogDetailPage({ params }) {
  ...
  return (
    <PublicShell>
      ...
      />
      <article className="grid gap-[12px]">
        <div className="grid items-start gap-[12px] lg:grid-cols-[minmax(0,1fr)_310px]">
          <div className="relative overflow-hidden rounded-[12px] bg-[var(--card)]">
            ...
            <div className="p-[clamp(20px,4vw,34px)]">
              <p className="mb-3 text-[14px] font-bold uppercase tracking-[0.04em] text-[var(--text-soft)]">
              ...
              <p className="max-w-[760px] text-[14px] text-[var(--text-soft)] sm:text-base">
                {post.excerpt}
              </p>
              ...
              {(post.tags || []).length ? (
                <div className="mt-5 flex flex-wrap gap-3">
                  {(post.tags || []).map((tag) => (
                    <span
                      className="inline-flex rounded-full bg-[var(--card-soft)] px-2 py-1.5 text-[14px] leading-none text-[var(--text-soft)]"
                      key={tag}
                    >
                      #{tag}
                    </span>
          ...
          <aside className="grid gap-3 lg:sticky lg:top-[20px]">
            <div className="rounded-[12px] bg-[var(--card)] p-[10px]">
              <form action="/blog" className="relative">
                <input
                  className="h-[44px] w-full rounded-3 bg-[var(--card-soft)] pl-3 pr-10 text-xs text-[var(--text)] outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]"
                  type="search"
                  name="search"
                ...
                <button
                  type="submit"
                  className="absolute right-2 top-1/2 inline-flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-[8px] bg-[var(--page)] text-[var(--text)] transition-colors duration-150 hover:bg-[var(--accent)] hover:text-white"
                  aria-label="Search"
                >
              ...
              {recentPosts
                ...
                .map((item) => (
                  <Link
                    ...
                    href={`/blog/${item.slug}`}
                    className="flex min-h-[76px] flex-col justify-between rounded-[12px] bg-[var(--card)] px-3 py-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]"
                  >
                    <p
                      className="text-[18px] font-bold lowercase leading-[1.35] text-[var(--text)]"
                      style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}
                    >
                      {item.title}
                    </p>
                    <p className="mt-4 text-[14px] leading-none text-[var(--text-faint)]">
